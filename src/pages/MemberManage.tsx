...
import { useMemberManage } from "../context/MemberManageContext";
...
export default function MemberManage() {
  ...
  }, [members, query, cohortFilter]);

  const presidentTotal = presidentCount;
  ...
  const confirmCopy: Record<ConfirmTarget["type"], { title: string; desc: string; action: string }> = {
    ...
    requestKick: {
      title: "강퇴를 요청하시겠습니까?",
      desc: `회장단 전원(${presidentTotal}명)이 동의해야 실제로 강퇴됩니다.`,
      action: "요청할게요",
    },
  };
      ...
      <div>
        ...
        <div className="mb-6 grid gap-3 sm:grid-cols-3">
          ...
          <Card>
            <p className="text-xs text-mute">
              {currentCohort} 신입 · 회장단 {presidentTotal}명
            </p>
        ...
        {kickRequests.length > 0 && (
          <Card className="mb-6 border-red-400/30">
            <p className="font-display text-lg text-backstage">⚠️ 강퇴 요청 진행 중</p>
            <div className="mt-3 space-y-2">
              {kickRequests.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-line bg-stage px-4 py-3"
                  >
                    <div>
                      <p className="text-sm font-semibold text-backstage">{r.targetName} 강퇴 요청</p>
                      <p className="mt-0.5 font-mono text-xs text-mute">
                        {r.requestedBy}님이 요청 · {r.requestedAt} · 동의 {r.approvedBy.length}/{presidentTotal}
                      </p>
